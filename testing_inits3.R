...
init <- matrix(NA, nrow = dim(CH_all)[1], ncol = 10)
obs.mat.init <- array(NA, dim = c(dim(CH_all)[1], 10, 2))
...
for (i in 1:nrow(Zinits)) {
  if (age[i, first[i]] == 1) {
    ...
    if (ever_observed) {
      ...
      Zinits[i, min(inds_2plus[1]+1, dim(CH_all[,-1])[2]):(dim(CH_all[,-1])[2])] <- 9
...
save(dat_marginal, const_marginal, file = "IPM_AEB_dat_stateSpace_marginal_loaf.RData")
...
save(inits_marginal, file = "IPM_AEB_inits_stateSpace_marginal_loaf.RData")
...
save(dat_marginal, const_marginal, file = "IPM_AEB_dat_stateSpace_marginal_loaf_reduced.RData")
...
save(inits_marginal, file = "IPM_AEB_inits_stateSpace_marginal_loaf_reduced.RData")
