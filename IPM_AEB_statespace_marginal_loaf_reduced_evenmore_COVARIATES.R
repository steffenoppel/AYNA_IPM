...
sel2 <- const_marginal$mult == 1
dat_marginal <- append(
  dat_marginal[-c(1:3)], 
  list(Y = dat_marginal$Y[sel2, ], 
  init = dat_marginal$init[sel2, ], 
  obs.mat.init = dat_marginal$obs.mat.init[sel2, ,]))
const_marginal <- append(
  const_marginal[-c(5:7, 9)],
  list(ageCat = const_marginal$ageCat[sel2, ], 
       n.inds = dim(dat_marginal$Y)[1],
       first = const_marginal$first[sel2],
       mult = const_marginal$mult[sel2]))
