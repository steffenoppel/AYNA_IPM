...
#### LOAD DATA ####
load("IPM_AEB_dat_stateSpace_marginal_loaf.RData") 

#### LOAD INITS #####
load("IPM_AEB_inits_stateSpace_marginal_loaf.Rdata") 
...
code <- nimbleCode({
  ...
  for (i in 1:n.inds) {
    ...
    Y[i, first[i]:n.occasions] ~ dDHMMo(init = init[i, 1:10], # initial state probabilities, provide this as data (1s and 0s, assuming first state known)
    ...
    for (t in (first[i]+1):n.occasions) {
      ...
      # 10. DEAD
      trans.mat[i,10,1,t] <- 0 
      trans.mat[i,10,2,t] <- 0
      trans.mat[i,10,3,t] <- 0
      trans.mat[i,10,4,t] <- 0
      trans.mat[i,10,5,t] <- 0
      trans.mat[i,10,6,t] <- 0
      trans.mat[i,10,7,t] <- 0
      trans.mat[i,10,8,t] <- 0
      trans.mat[i,10,9,t] <- 0
      trans.mat[i,10,10,t] <- 1
      ...
      # breed in colony
      obs.mat[i,8, 1,t] <- 1-p.det[t-1]
      obs.mat[i,8, 2,t] <- p.det[t-1]
...
save(out1, file = "samples_statespace_marginal_loaf.Rdata")
