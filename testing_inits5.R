...
for (age in 8:maxage) {
  ...
  # AEB change
  IMinits[1, age, 1] = rbinom(1, IMinits[1, age-1, 3], imean.phi.im*(1+iinflation.factor))
...
for (tt in 2:n.years.fec) {
  ...
  for (age in 2:maxage) {
    IMinits[tt, age, 1] <- rbinom(1, IMinits[tt-1, age-1, 3], imean.phi.im*(1+iinflation.factor))
...
diffTimeObs <- cbind(apply(AYNA_CHICK_reduced[, -38], 1, function(x){min(which(x==1))}), 
                     apply(AYNA_CHICK_reduced[, -38], 1, function(x){which(x==1)[2]})) %>% 
  as.data.frame() %>% 
...
obs.mat.init <- array(NA, dim = c(dim(CH_all_reduced)[1], 6, 2))
...
save(dat_marginal, const_marginal, file = "IPM_AEB_dat_stateSpace_marginal_loaf_reduced_incolony_COVARIATES.RData")
...
save(inits_marginal, file = "IPM_AEB_inits_stateSpace_marginal_loaf_reduced_incolony_COVARIATES.RData")
