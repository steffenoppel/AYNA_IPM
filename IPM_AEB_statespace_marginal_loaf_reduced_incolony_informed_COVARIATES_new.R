...
#### LOAD DATA ####
load("IPM_AEB_dat_stateSpace_marginal_loaf_reduced_incolony_informed_COVARIATES.RData") 

#### LOAD INITS #####
load("IPM_AEB_inits_stateSpace_marginal_loaf_reduced_incolony_informed_COVARIATES.Rdata") 
...
code <- nimbleCode({
  ...
  for(age in 8:maxAge) {
    IM[1,age,1] ~ dbin(mean.phi.ad*(1+inflation.factor), IM[1,age-1,3])
  ...
  for (tt in 2:n.years.fec){
    ...
    for(age in 2:maxAge) {
      IM[tt,age,1] ~ dbin(phi.ad[tt+offset-1]*(1+inflation.factor), IM[tt-1,age-1,3])
  ...
  for (i in 1:n.inds) {
    ...
    Y[i, first[i]:n.occasions] ~ dDHMMo_mod(init = init[i, 1:6], # initial state probabilities, provide this as data (1s and 0s, assuming first state known)
                                            probObs = obs.mat[i, 1:6, 1:4, first[i]:n.occasions], # time dependent 3d array (index by i) [nstates, nevents, t]; assume known at first and provide as data
    ...
    for (t in (first[i]+1):n.occasions) {
      ...
      trans.mat[i,2,3,t] <- phi[i,t]*(1-p.atsea[i,t])*(1-p.propensity[i,t])
      trans.mat[i,2,4,t] <- 0
      trans.mat[i,2,5,t] <- phi[i,t]*(1-p.atsea[i,t])*(p.propensity[i,t])
      trans.mat[i,2,6,t] <- 1-phi[i,t]
      
      # 3. LOAF IN (HAS BRED)
      trans.mat[i,3,1,t] <- 0 
      trans.mat[i,3,2,t] <- phi[i,t]*p.atsea[i,t]
      trans.mat[i,3,3,t] <- phi[i,t]*(1-p.atsea[i,t])*(1-p.propensity[i,t])
      trans.mat[i,3,4,t] <- 0
      trans.mat[i,3,5,t] <- phi[i,t]*(1-p.atsea[i,t])*(p.propensity[i,t])
      trans.mat[i,3,6,t] <- 1-phi[i,t]
      
      # 4. RECRUIT IN
      trans.mat[i,4,1,t] <- 0 
      trans.mat[i,4,2,t] <- phi[i,t]*p.atsea[i,t]
      trans.mat[i,4,3,t] <- phi[i,t]*(1-p.atsea[i,t])*(1-p.propensity[i,t])
      trans.mat[i,4,4,t] <- 0
      trans.mat[i,4,5,t] <- phi[i,t]*(1-p.atsea[i,t])*(p.propensity[i,t])
      trans.mat[i,4,6,t] <- 1-phi[i,t]
      
      # 5. BREED IN
      trans.mat[i,5,1,t] <- 0 
      trans.mat[i,5,2,t] <- phi[i,t]*p.atsea[i,t]
      trans.mat[i,5,3,t] <- phi[i,t]*(1-p.atsea[i,t])*(1-p.propensity[i,t])
      trans.mat[i,5,4,t] <- 0
      trans.mat[i,5,5,t] <- phi[i,t]*(1-p.atsea[i,t])*(p.propensity[i,t])
      trans.mat[i,5,6,t] <- 1-phi[i,t]
      ...
      # events
      # 1 - seen breeding
      # 2 - seen loafing (has bred)
      # 3 - seen unknown (could be repeat breeding, loafing, or a recruit)
      # 4 - unseen 
      
      # prebreeder
      obs.mat[i,1, 1,t] <- 0
      obs.mat[i,1, 2,t] <- 0
      obs.mat[i,1, 3,t] <- 0 #p.det[t-1]
      obs.mat[i,1, 4,t] <- 1 #1-p.det[t-1]
      ...
      obs.mat[i,4, 2,t] <- 0 #0
      obs.mat[i,4, 3,t] <- (1-gamma[t]) #p.det[t-1]*(1-gamma[t])
      obs.mat[i,4, 4,t] <- 0 #1-p.det[t-1]
      
      # breed in colony
      obs.mat[i,5, 1,t] <- gamma[t] #p.det[t-1]*gamma[t]
      obs.mat[i,5, 2,t] <- 0 #0
      obs.mat[i,5, 3,t] <- (1-gamma[t]) #p.det[t-1]*(1-gamma[t])
      obs.mat[i,5, 4,t] <- 0 #1-p.det[t-1]
