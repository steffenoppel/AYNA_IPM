...
#-------------------------------------------------
  ...
  for (tt in (T+1):FUT.YEAR){
    ...
    ## RANDOMLY DRAW DEMOGRAPHIC RATES FROM PREVIOUS YEARS 
    ...
    # -------------------------------------------------
      ...
      N.notrecruited[tt] ~ dbin(ann.surv[2,FUT.int[tt]-1], round(max(1,non.recruits[tt-1])))       ### number of not-yet-recruited birds surviving from previous year
      non.recruits[tt]<-(N6[tt]+N.notrecruited[tt])-ann.recruits[tt]                      ## number of birds that do not recruit is the sum of all available minus the ones that do recruit
      ann.recruits[tt] ~ dbin(imm.rec[FUT.int[tt]],N.notrecruited[tt])                    ### new recruits
...
# Call JAGS from R
AYNApopmodel <- jags(jags.data, inits, parameters, "C:\\STEFFEN\\RSPB\\UKOT\\Gough\\ANALYSIS\\AYNA_IPM\\AYNA_IPM_projection.jags", n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb,parallel=T)
