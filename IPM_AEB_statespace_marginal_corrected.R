...
#### LOAD DATA ####
load("IPM_AEB_dat_stateSpace_marginal_fixed.RData") 

#### LOAD INITS #####
load("IPM_AEB_inits_stateSpace_marginal_fixed.Rdata") 
...
code <- nimbleCode({
  ...
  mean.p.fidelity ~ dbeta(5,2) # probability individual breeds in monitored area
  mean.p.propensity ~ dbeta(5,2) # rebreeding propensity 
  ...
  for (tt in 2:n.years.fec){
    for (age in 1:maxAge) {
      logit(p.juv.recruit[age,tt]) <- mu.p.juv + agebeta * sqrt(age) # + eps.p[tt+offset-1]
    }
    ...
    N.breed.ready[tt] ~ dbin(mean.p.propensity, N.ad.surv[tt]) ### number of available breeders is proportion of survivors that returns
  ...
  for (i in 1:n.inds) {
    ...
    Y[i, first[i]:n.occasions] ~ dDHMMo(init = init[i, 1:8], # initial state probabilities, provide this as data (1s and 0s, assuming first state known)
                                        probObs = obs.mat[i, 1:8, 1:2, first[i]:n.occasions], # time dependent 3d array (index by i) [nstates, nevents, t]; assume known at first and provide as data
                                        probTrans = trans.mat[i, 1:8, 1:8, (first[i]+1):n.occasions], # time dependent 3d array (index by i) [nstates, nstates, t]
                                        len = n.occasions - first[i] + 1, # length of observations
                                        checkRowSums = 0) 
    
    obs.mat[i,1:8, 1:2, first[i]] <- obs.mat.init[i, 1:8, 1:2]
    ...
    for (t in (first[i]+1):n.occasions) {
      ...
      trans.mat[i,1,2,t] <- 0
      trans.mat[i,1,3,t] <- 0
      trans.mat[i,1,4,t] <- phi[i,t]*p.recruit[i,t]*p.fidelity[i,t]
      trans.mat[i,1,5,t] <- phi[i,t]*p.recruit[i,t]*(1-p.fidelity[i,t])
      trans.mat[i,1,6,t] <- 0
      trans.mat[i,1,7,t] <- 0
      trans.mat[i,1,8,t] <- 1-phi[i,t]
      ...
      trans.mat[i,2,4,t] <- 0
      trans.mat[i,2,5,t] <- 0
      trans.mat[i,2,6,t] <- phi[i,t]*p.propensity[i,t]*p.fidelity[i,t]
      trans.mat[i,2,7,t] <- phi[i,t]*p.propensity[i,t]*(1-p.fidelity[i,t])
      trans.mat[i,2,8,t] <- 1-phi[i,t]
      
      trans.mat[i,3,1,t] <- 0
      trans.mat[i,3,2,t] <- 0
      trans.mat[i,3,3,t] <- phi[i,t]*(1-p.propensity[i,t])
      trans.mat[i,3,4,t] <- 0
      trans.mat[i,3,5,t] <- 0
      trans.mat[i,3,6,t] <- phi[i,t]*p.propensity[i,t]*(1-p.fidelity[i,t])
      trans.mat[i,3,7,t] <- phi[i,t]*p.propensity[i,t]*(p.fidelity[i,t])
      trans.mat[i,3,8,t] <- 1-phi[i,t]
      ...
      trans.mat[i,4,4,t] <- 0
      trans.mat[i,4,5,t] <- 0
      trans.mat[i,4,6,t] <- phi[i,t]*p.propensity[i,t]*p.fidelity[i,t]
      trans.mat[i,4,7,t] <- phi[i,t]*p.propensity[i,t]*(1-p.fidelity[i,t])
      trans.mat[i,4,8,t] <- 1-phi[i,t]
      
      trans.mat[i,5,1,t] <- 0
      trans.mat[i,5,2,t] <- 0
      trans.mat[i,5,3,t] <- phi[i,t]*(1-p.propensity[i,t])
      trans.mat[i,5,4,t] <- 0
      trans.mat[i,5,5,t] <- 0
      trans.mat[i,5,6,t] <- phi[i,t]*p.propensity[i,t]*(1-p.fidelity[i,t])
      trans.mat[i,5,7,t] <- phi[i,t]*p.propensity[i,t]*(p.fidelity[i,t])
      trans.mat[i,5,8,t] <- 1-phi[i,t]
      ...
      trans.mat[i,6,4,t] <- 0
      trans.mat[i,6,5,t] <- 0
      trans.mat[i,6,6,t] <- phi[i,t]*p.propensity[i,t]*(p.fidelity[i,t])
      trans.mat[i,6,7,t] <- phi[i,t]*p.propensity[i,t]*(1-p.fidelity[i,t])
      trans.mat[i,6,8,t] <- 1-phi[i,t]
      
      trans.mat[i,7,1,t] <- 0
      trans.mat[i,7,2,t] <- 0
      trans.mat[i,7,3,t] <- phi[i,t]*(1-p.propensity[i,t])
      trans.mat[i,7,4,t] <- 0
      trans.mat[i,7,5,t] <- 0
      trans.mat[i,7,6,t] <- phi[i,t]*p.propensity[i,t]*(1-p.fidelity[i,t])
      trans.mat[i,7,7,t] <- phi[i,t]*p.propensity[i,t]*(p.fidelity[i,t])
      trans.mat[i,7,8,t] <- 1-phi[i,t]
      ...
      # breed in colony
      obs.mat[i,6, 1,t] <- 1-p.det[t-1]
      obs.mat[i,6, 2,t] <- p.det[t-1]
...
params <- c(
  ...
  # SURVIVAL
  "mean.p.fidelity", "mean.p.propensity", "mean.p.det",
  "mu.p.juv", "agebeta", "sigma.p", "eps.p",
  "mean.phi.juv", "mean.phi.ad", "sigma.phi", "eps.phi",
  
  # ABUNDANCE
  "IM", "N.recruits", "N.ad.surv", "N.breed.ready",
  "Ntot.breed", "N.atsea", "nestlings", "JUV", "Ntot"
)
...
save(out1, file = "samples_statespace_marginal_fixed.Rdata")
...
summ_backhalf_bychain <- post_summ(out1_backhalf, get_params(out1_backhalf, type = 'base_index'),
                                                                 by_chain = TRUE, probs = c(0.025, 0.5, 0.975))
test <- apply(summ_backhalf_bychain, c(1,3), t)
test <- cbind(summ_backhalf[,1], test[,1:2,1], test[,1:2,2], test[,1:2,3]) %>% 
  as.data.frame()
